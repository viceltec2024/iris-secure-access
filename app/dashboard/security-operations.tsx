...
type DeviceTelemetry = { hostname?: string; osVersion?: string; architecture?: string; diskUsedPercent?: number; memoryUsedPercent?: number; firewallEnabled?: boolean; collectedAt?: string };
...
export default function SecurityOperations({ user, auditCount, signOutPath }: { user: { email: string; displayName: string; role: string }, auditCount: number, signOutPath: string }) {
  ...
  return <main className="soc-shell">
    ...
    <section className="soc-main">
      ...
      {section === "devices" && <section className="module-panel devices-view"><div className="module-toolbar"><div><h2>{language === "es" ? "Inventario real de dispositivos" : "Real device inventory"}</h2><p>{language === "es" ? "IRIS solo muestra como REAL la información reportada por un agente autorizado." : "IRIS marks information REAL only when an authorized agent reports it."}</p></div><button className="add-device" onClick={createDeviceEnrollment} disabled={creatingDevice}><Plus />{creatingDevice ? (language === "es" ? "Creando…" : "Creating…") : (language === "es" ? "Registrar mi Mac" : "Register my Mac")}</button></div><div className="device-grid">{devices.map(device => <article key={device.id}><div className="device-card-head"><div className="device-icon"><Desktop weight="duotone" /></div><div><strong>{device.name}</strong><span>{device.telemetry?.hostname || device.platform}</span></div><b className={`provenance-badge ${device.provenance.toLowerCase()}`}>{device.provenance === "REAL" ? "REAL" : (language === "es" ? "NO VERIFICADO" : "UNVERIFIED")}</b></div><div className="device-status-line"><b className={`device-state ${device.status.toLowerCase()}`}>{device.status === "PENDING" ? (language === "es" ? "PENDIENTE" : "PENDING") : device.status}</b><strong>{language === "es" ? "Salud" : "Health"}: {device.healthScore ?? "—"}{device.healthScore !== null && "/100"}</strong></div><dl><div><dt>{language === "es" ? "Firewall" : "Firewall"}</dt><dd>{device.telemetry ? (device.telemetry.firewallEnabled ? (language === "es" ? "ACTIVO" : "ON") : (language === "es" ? "DESACTIVADO" : "OFF")) : "—"}</dd></div><div><dt>{language === "es" ? "Sistema" : "System"}</dt><dd>{device.telemetry ? `${device.telemetry.osVersion || "macOS"} · ${device.telemetry.architecture || "—"}` : "—"}</dd></div><div><dt>{language === "es" ? "Disco usado" : "Disk used"}</dt><dd>{device.telemetry?.diskUsedPercent ?? "—"}{device.telemetry?.diskUsedPercent !== undefined && "%"}</dd></div><div><dt>{language === "es" ? "Memoria usada" : "Memory used"}</dt><dd>{device.telemetry?.memoryUsedPercent ?? "—"}{device.telemetry?.memoryUsedPercent !== undefined && "%"}</dd></div><div><dt>{language === "es" ? "Último reporte" : "Last report"}</dt><dd>{device.lastSeenAt ? new Date(device.lastSeenAt).toLocaleString(language) : (language === "es" ? "Nunca" : "Never")}</dd></div></dl><div className="enrollment-code"><span>{language === "es" ? "Código de inscripción" : "Enrollment code"}</span><code>{device.enrollmentCode}</code></div><div className="device-actions"><a href="/iris-agent-macos.sh" download>{language === "es" ? "Descargar agente" : "Download agent"}</a><button onClick={() => void rotateEnrollmentCode(device.id)}>{language === "es" ? "Generar código nuevo" : "Generate new code"}</button><button className="delete-device" disabled={deletingDeviceId === device.id} onClick={() => void deleteDevice(device)}><Trash weight="bold" />{deletingDeviceId === device.id ? (language === "es" ? "Eliminando…" : "Deleting…") : (language === "es" ? "Eliminar dispositivo" : "Delete device")}</button></div><p><Warning weight="fill" />{device.status === "ONLINE" ? (language === "es" ? "Telemetría real, reciente y de solo lectura." : "Real, recent, read-only telemetry.") : device.status === "OFFLINE" ? (language === "es" ? "El agente dejó de reportar hace más de 5 minutos." : "The agent stopped reporting more than 5 minutes ago.") : (language === "es" ? "Sin telemetría: estos datos no están verificados." : "No telemetry: this device is unverified.")}</p></article>)}{devices.length === 0 && <div className="empty-devices"><Desktop weight="duotone" /><h3>{language === "es" ? "No hay dispositivos conectados" : "No connected devices"}</h3><p>{language === "es" ? "Registra tu Mac para generar un código seguro. La supervisión comenzará únicamente después de instalar y autorizar el agente." : "Register your Mac to generate a secure code. Monitoring will begin only after you install and authorize the agent."}</p></div>}</div></section>}
